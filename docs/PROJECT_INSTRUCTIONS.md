# loseit-cli project instructions
...
- `internal/nutrition/` — per-day aggregation = the `days --json` contract (banker's rounding; see docs/MACHINE_CONTRACT.md).
...
- Install Go and put it on PATH; derive helper locations from go env GOPATH (its bin directory) rather than an owner-specific path.
...
- `docs/MACHINE_CONTRACT.md` — machine contract (command shapes, JSON, exit codes, auth model).
...
The existing local review reconfirmed nutrition-only scope. Keep the downloaded ZIP fallback and avoid login retry loops that risk account lockout. Use an explicit config path for unattended calls; CWD config discovery can choose a token path. Windows uses inherited ACLs, so POSIX 0600/0700 are advisory there; protect the containing directory. Consumers should alert on repeated exit 2, keep the frozen rounded values, and upsert overlapping date windows idempotently. Do not import consumer storage code here.
